...
void GeoSgdCommunicator::Send(const std::vector<std::string> &sparse_var_names,
  ...
  for (size_t i = 0; i < sparse_var_tables.size(); i++) {
    ...
    current_sparse_var.resize(element_number);
    ...
    VLOG(4) << "ids_vec sparse var: " << sparse_var_names[i] << " size "
...
void GeoSgdCommunicator::SparseIdsMerge(SparseIdsVec *ids_send_vec) {
  ...
  for (auto &sparse_table : (*ids_send_vec)) {
    ...
    for (auto &sparse_var : sparse_table.second) {
      VLOG(3) << "sparse_var size " << sparse_var.size();
      auto merge_task = [this, &sparse_table_name, &sparse_var] {
        VLOG(3) << "sparse_table_name" << sparse_table_name;
        VLOG(3) << "In task sparse_var size " << sparse_var.size();
...
void GeoSgdCommunicator::RecvUpdateDenseVars(
  ...
  VLOG(1) << "dense var update " << origin_splited_var_name << " use time "
